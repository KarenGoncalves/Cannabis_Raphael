...
#### Prepare environment ####
source("FUNCTIONS.R") # loads packages too
args = commandArgs(trailingOnly=T)
cores = ifelse(length(args) != 1, detectCores(), as.numeric(args[1]))
edgetable_dir="Main_analysis_edge_table_March2026"

load("GeneSelection_objects.RData")
...
metadata <- read_delim("metadata_pca.txt") %>%
...
edgeTableFiles = 
  list.files(pattern = "edge_table_r",
             path = edgetable_dir, full.names=T)
...
funct_anno = read_delim("Annotation_expressed_genes.txt")
...
ggsave("Optimize_resolution.svg", 
...
for (curResolution in resolutions) {
  ...
  paste0("MainAnalysis_Cor", r_cutoff) %>% dir.create()
  save(Expr_averaged_z_high_var_modules,
       module_peak_exp, modules_mean_z, my_network_modules,
       file = paste0(
         "MainAnalysis_Cor", r_cutoff, "/ModuleData_forPlot_res",
         curResolution, ".RData")
  )
}
