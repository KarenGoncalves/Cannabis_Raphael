...
if(nrow(Exp_table_long_averaged_z) < split_at) {
    Exp_table_long_averaged_z %>% 
        mutate(group = 0) %>%
        write_delim(file = paste0(basedir, "/Exp_table_long_averaged_z_0.tsv"),
                    quote = "none", append = F,
                    col_names = T, delim = "\t")
} else {
    Exp_table_long_averaged_z %>% 
        ...
        group_split(group) %>%
        map(.f = ~{
            fileName = paste0(basedir, "/Exp_table_long_averaged_z_",
                              unique(.x$group), ".tsv")
...
write_delim(all_coefVar_and_ranks,
            paste0(basedir, "/all_coefVar_and_ranks.tsv"),
...
bait_var <- high_var_genes_pct %>% 
    ...
    group_by(gene_ID)
right_join(all_coefVar_and_ranks, 
           Baits, by="gene_ID")
...
#        height = 5, width = 5)
ggsave(plot = var_plot, 
       filename = paste0(basedir, "/gene_relative_variance_distribution.svg"), 
       height = 5, width = 7)
...
write_delim(high_var_genes_pct,
            file = paste0(basedir, "/high_coefvar_genes_", 
                          pct_genes, "pct.tsv"),
