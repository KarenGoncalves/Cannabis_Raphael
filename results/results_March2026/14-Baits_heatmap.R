...
baits <- 
    read_excel("BaitGenes_RNA_CoExprAnalysis.xlsx", sheet = "Bait_genes_Ensembl_ID") %>% 
...
tpm_baits = read_delim("results/results_March2026/Exp_table_long_averaged_z_0.tsv") %>%
...
read_delim("results/results_March2026/Exp_table_long_averaged_z_0.tsv") %>% 
    inner_join(baits, by = "gene_ID") %>% 
    group_by(gene_ID, Symbol) %>% 
    summarize(minTPM = min(mean.TPM),
              max_TPM=max(mean.TPM))
...
ggsave("results/results_March2026/Baits_heatmap_byName.png",
       height = 6.2, width = 4.4,
       dpi=600)

ggsave("results/results_March2026/Baits_heatmap_byName.svg",
       height = 6.2, width = 4.4)
...
ggsave("results/results_March2026/Baits_heatmap_Pathway.svg",
       height = 6, width = 4.7)
ggsave("results/results_March2026/Baits_heatmap_Pathway.png",
       height = 6, width = 4.7,
       dpi=1200)
